package com.paliy.gymcounter_test_04;

class StaticData {
    public static final float FAST_H = 7.3f;
    public static final float FAST_L = -6.5f;
    public static final float AVERAGE_H = 5.6f;
    public static final float AVERAGE_L = -4.0f;
    public static final float SLOW_H = 3.5f;
    public static final float SLOW_L = -3.0f;

    public static final int PLUS_VALUE = 5;

}

